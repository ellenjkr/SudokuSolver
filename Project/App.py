...
class App():
	...
	def monta_grafo(self, T, vertices):
		...
		nx.draw(self.G, pos, with_labels=False, arrows=False, node_color=list(node_colors.values()), node_size=node_size, edgecolors="black", width=edge_width, ax=self.ax)  # Desenha o grafo
		
		# Adiciona os números do sudoku invés dos nomes de cada vértice
		labels = {}
		for node in self.G.nodes():
			labels[node] = str(vertices[node - 1].cor)
		nx.draw_networkx_labels(self.G, pos, labels, font_size=font_size, ax=self.ax)
